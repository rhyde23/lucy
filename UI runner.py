...
def rearrange_lineup_formation_change(new_formation, new_positions) :
    ...
    while True :
        ...
        for new_position in new_positions :
            ...
            if current_data[team+'_Players'][best_options[0]]['Position'] == converted_pos :
                best_options_each_pos = {}
    ...
    highest_combo, highest_rating = [], 0
    for p in itertools.permutations(left_players) :
        attempt = determine_rating_of_combo(p, left_positions_order, current_lineup_position_ratings, number_in_combo)
        if attempt > highest_rating :
            highest_combo, highest_rating = p, attempt
    for i in range(number_in_combo)  :
        final_new[left_positions_order[i]] = highest_combo[i]
    ...
    for i in range(11) :
        ...
        if real_position[0] in ['R', 'L'] :
            if current_pos[0] != real_position[0] :
                if current_pos[0] == 'C' :
                    possibility_build = real_position[0]+current_pos
                else :
                    possibility_build = ''.join([real_position[0]]+list(current_pos)[1:])
                print(current_pos, current_player_in_lineup, possibility_build)
        ...
        """
        if real_position[0] == 'R' :
            current_rol, other_rol = 'R', 'L'
        elif real_position[0] == 'L' :
            current_rol, other_rol = 'L', 'R'
        else :
            continue
        print(current_player_in_lineup, current_pos, positions_in_order, lineup_in_order)
        current_broken_up = list(current_pos)
        current_broken_up[0] = other_rol
        other_pos = ''.join(current_broken_up)
        other_player_index = positions_in_order.index(other_pos)
        other_player = lineup_in_order[other_player_index]
        print(current_player_in_lineup, other_player)
        quit()
        """
